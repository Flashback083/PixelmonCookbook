...
import com.pixelmonmod.cookbook.examplemod.battle.Rules;
 ...
 *
 * Class is intended to show how you might start a battle in various ways.
 * @see Rules
 */
public class BattleStarter {
    ...
    public void StartBattleWithRules(EntityPlayerMP player1, EntityPlayerMP player2) {
        PlayerParticipant[] p1 = {new PlayerParticipant(player1, Pixelmon.storageManager.getParty(player1).getAndSendOutFirstAblePokemon(player1))};
        PlayerParticipant[] p2 = {new PlayerParticipant(player2, Pixelmon.storageManager.getParty(player2).getAndSendOutFirstAblePokemon(player2))};

        Rules rules = new Rules();
        rules.addClause(BattleClauseRegistry.BAG_CLAUSE);
        rules.addClause(BattleClauseRegistry.SKY_BATTLE);
        rules.levelCap = 100;
        rules.fullHeal = true;
    ...
    public void StartBattleQuery(EntityPlayerMP player1, EntityPlayerMP player2, boolean forceAccept){
        EntityPixelmon party1 = Pixelmon.storageManager.getParty(player1).getAndSendOutFirstAblePokemon(player1);
        EntityPixelmon party2 = Pixelmon.storageManager.getParty(player2).getAndSendOutFirstAblePokemon(player2);
        BattleQuery battlequery = new BattleQuery(player1, party1, player2, party2);
